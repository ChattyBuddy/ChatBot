...
with open(datafile, 'wb') as outputfile:
...
#with open(datafile, 'w') as outputfile:
    ...
    for pair in extractSentencePairs(conversations):
        ...
        print(type(pair[0]))'''
        try:
            #writer.writerow([str(pair[0]),str(pair[1])])
            writer.writerow(pair)
            pairs_accepted += 1
        except:
            pairs_denied += 1

print("Accepted: " + str(pairs_accepted))
print("Ignored: " + str(pairs_denied))
...
for pair in pairs[:10]:
    #Modified here
    #pair[0] = pair[0].decode()
    #pair[1] = pair[1].decode()
    #To here
    print(pair)
